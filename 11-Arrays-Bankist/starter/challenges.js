...
/* const checkDogs = function (dogsJulia, dogsKate) {
  ...
  const dogsJuliaAndKate = copyDogsJulia.concat(dogsKate);

  dogsJuliaAndKate.forEach((dogAge, i) => {
    if (dogAge < 3) {
...
/* const calcAverageHumanAge = function (dogAges) {
  ...
  const avgDogAge = adultDogs.reduce(
...
// Task #6
const checkEatingOkay = (cur, rec) => cur > cur * 0.9 && cur < rec * 1.1;

console.log(dogs.some(dog => checkEatingOkay(dog.curFood, dog.recFood)));

// Task #7
const dogsOkayFood = dogs.filter(dog =>
  checkEatingOkay(dog.curFood, dog.recFood)
);
